...
## The organ pulse — proprioception for the body's own activity

The three objects above are all graded from evidence the agent left behind. There is a prior blind spot: **most organs leave no evidence at all.** A skill invoked through a tool call has a trace; an organ that runs in prose — the contemplation cycle recalled from memory, the loop's integrate step, this organ's own cycle — leaves nothing. An untraced organ is indistinguishable from a dormant one, and the difference matters: the suite's own dormancy audit found a register that had fired *zero times in three months* while every session had loaded it. Loaded is not used, and nothing on the box could have said so earlier.

The pulse is the body's own activity, kept: **which organs actually fired, how often, and when last.** Not how long — no timer exists, and time-to-next-human-turn measures the human's absence, not the organ's work.

Mechanics:

1. **Trace every firing at the boundary where it can be seen.** A host with tool hooks records skill calls, organ-script runs and organ-doc reads mechanically (`scripts/organ-fired.py` in hook mode). That covers the invoked organs for free.
2. **Prose organs mark themselves.** A cycle run without a tool call records one line at its start: `organ-fired.py --mark <organ> "<why>"` — the marker every organ in the suite now carries. Put the *reminder* on the wake reflex (the script prints a one-line pulse at session start when bound there), not in memory: a marker that needs recall does not fire, which is the finding this layer exists to fix.
3. **Read the pulse at wake, next to the world-diff.** Per organ: firings in 7 and 30 days, last firing, time since. An organ silent for weeks is one of two things — *untraced* (add the marker where it runs) or *dormant* — and the pulse cannot tell them apart; deciding which is a proprioception question ("was it needed and skipped, or not needed?"), and the answer is written back like any other correction.
4. **Distinguish observed from reconstructed.** A host that kept transcripts can backfill; tag those rows. A reconstructed pulse is a prior, not a measurement.

**Fitness signal:** dormant organs caught by the pulse and either re-bound or deliberately retired — versus a ledger nobody reads, or one that records only the organs that were already visible.

`scripts/organ-fired.py` implements it (hook mode / `--mark` / `pulse`) over a plain TSV (`$ORGAN_LEDGER`, default `~/.organ-fired.tsv`). **Keep the real ledger private** — it holds session ids and command fragments; `examples/organ-fired.tsv` shows the shape with fabricated rows.
