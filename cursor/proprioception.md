...
All three objects above are graded from evidence the agent left behind, and most organs leave none: an invoked skill has a tool trace, an organ run in prose (the contemplation cycle recalled from memory, the loop's integrate step, this cycle itself) has nothing — so **untraced is indistinguishable from dormant**, and the suite's own audit once found a register that had fired zero times in three months while every session had loaded it. The pulse keeps the body's own activity: which organs fired, how often, when last (not how long — no timer exists, and time-to-next-human-turn measures the human's absence). Trace invoked organs mechanically at the tool boundary (`scripts/organ-fired.py` in hook mode); have prose organs mark themselves at the start of a cycle (`organ-fired.py --mark <organ> "<why>"`, the line every suite organ now carries) with the *reminder* bound to the wake reflex, never to memory; read the pulse at wake next to the world-diff; treat a weeks-silent organ as either untraced (add the marker) or dormant, decide which, and write the answer back. Tag reconstructed rows as such — a backfilled pulse is a prior, not a measurement. Fitness: dormant organs caught and re-bound or retired vs. a ledger nobody reads. Keep the real ledger private; `examples/organ-fired.tsv` is the fabricated shape.
